...
- Increased the timeout and added retries for the USB accessory-mode handshake, for phones that are slow to respond right after being plugged in
- Fixed a possible USB accessory-mode conflict when more than one part of the app tried to switch the same device at once
- Improved detection of phones connected in "charging only" USB mode
- Fixed: Video decoder could crash during the frozen-screen recovery in rare cases
- Fixed: Poor Wi-Fi conditions (e.g. 2.4GHz Wi-Fi + Bluetooth) could trigger unnecessary decoder restarts that made lag worse instead of better
- Fixed: Video keyframes could be silently corrupted after a decoder restart on some chipsets (Rockchip/Allwinner/Telechips), causing recurring glitches
- Reduced how long the video decoder can block the network thread when its buffers are full, improving resilience on slow/congested connections
- Fixed: Audio delay could grow during network hiccups and never recover for the rest of the session; lowered the default audio buffer and capped how much audio can queue up
- Fixed: Wi-Fi Direct now prefers 5GHz for wireless connections, fixing severe lag when Bluetooth and Wi-Fi were forced onto the same 2.4GHz band
- Fixed: App would silently reconnect right after pressing Exit in Android Auto instead of staying disconnected
- Fixed: Several Wi-Fi Direct reliability issues - retry counters not resetting between attempts, a band-mismatch retry that could drop an already-active session, and leftover/stale groups left in a broken state
- Fixed: Reconnecting overlay and frozen-screen recovery could silently stop working for the rest of a session
- Fixed: Wi-Fi lock, GPS and audio focus could stay held indefinitely if the very first connection attempt failed its handshake
- Fixed: Native AA Bluetooth handshake server could get stuck permanently disabled if Bluetooth was off at startup
- Fixed: Various connection-layer leaks (sockets/file descriptors on reconnect loops, USB stream desync after a read timeout, Google Nearby tunnel getting stuck after a reconnect)
- Fixed: Media metadata (title/artist/album art) could stop updating for the rest of a session after unusually large album art
- Fixed: Duplicate steering-wheel/hardware-button broadcast receiver causing doubled key presses
- Fixed: Native AA Bluetooth "poke" could repeat and disturb an in-progress handshake
- Fixed: Native AA no longer exits the session on a single transient video-focus handoff (e.g. incoming call, Assistant popup) - requires it to repeat before treating it as a real exit
- Fixed: Possible crash on FYT head units if the vendor service disconnects mid-setup, and a crash on shutdown in some service-start-failure cases
- General reliability pass across the wireless/USB connection layer and AAP protocol handling
- Security: unrecognized Wi-Fi/Bluetooth/Nearby devices now need a one-tap confirmation (via notification) before the app connects to them, instead of auto-connecting to anything found on the network or paired over Bluetooth; previously-approved devices keep auto-connecting as before
- Security: hotspot password and Wi-Fi Direct network name/password are no longer generated with a predictable random source
- Security: a crafted deep link could make the app connect straight to an attacker-chosen address with no confirmation
- Security: media-button broadcasts from other apps could inject arbitrary key codes into an active session; now limited to standard media transport keys
- Security: removed an unused, insecure trust-all SSL helper left over from an old implementation
- Fixed: Settings screen could silently discard unsaved changes if it got recreated (e.g. right after toggling dark mode) before you hit Save
- Fixed: Switching a USB device into Android Auto mode from the device list could freeze the whole UI for several seconds
- Fixed: The auto-connect indicator could get permanently stuck off after certain app-exit timings, silently disabling auto-connect until the app was killed and restarted
- Fixed: "Kill app on disconnect" could close the whole app just from cancelling a connection attempt, not only from an actual Android Auto disconnect
- Fixed: Resetting all settings didn't fully clear Bluetooth auto-start data, so auto-start could keep working after a reset
- Fixed: Selecting a device from the Google Nearby list could occasionally connect to the wrong device if the list updated at that exact moment
- Fixed a few crash risks: invalid stored night-mode/view-mode values, and the keymap screen's key listener
- Fixed: exported/file-based logs were silently dropping error stack traces; also fixed a couple of log-capture race conditions
- Fixed: picking a new loading-screen image/video that failed validation (too large/unsupported) no longer deletes your existing one
- Various smaller reliability fixes across Settings, the Wi-Fi network list, and Bluetooth auto-start handling
- Security: closed a second, independent path that could auto-connect to an unapproved network device - the manual "Scan" button in the Wi-Fi list had its own connect logic that bypassed the approval system entirely; reconnecting to your last-used server on launch is now also checked against the approved list
- Security: Google Nearby's approval prompt now shows a confirmation code to match against your phone, since the device name alone can be spoofed by another device nearby
- Security: a restored settings backup could silently re-add a Bluetooth device to the trusted auto-start list, bypassing the one-tap approval prompt; that list is no longer included in backups
- Security: fixed a race where rapid connection attempts from different devices could show the wrong approval prompt, or suppress a legitimate one
- Security: deep links and connection approvals now reject non-IP hostnames, closing a possible DNS-rebinding trick
- Security: the "disconnect"/"exit" deep links can no longer be triggered from a web page, only from other apps on the device
- Security: reduced how often unofficial ACC-on/quick-boot broadcasts (used by some Chinese head units) can repeatedly force-start the app
- Security: removed an unnecessary export flag on the Android Auto projection screen and tightened a few other component/broadcast settings
- Fixed: a quick pause/resume (or navigation-prompt ducking) could permanently silence media audio for the rest of the session when Static Audio Focus mode is enabled
- Fixed: audio delay after a network stall could get "baked in" permanently instead of catching back up over time
- Fixed a native audio-effect handle leak, and a rare case where a failed audio mixer start could leave audio broken for the rest of the session
- Fixed: Wi-Fi auto-start could silently never trigger while the app was in the background on Android 10+, due to a location-permission SSID restriction
- Fixed the DPI preview screen showing the wrong panel aspect ratio on Android 11+
