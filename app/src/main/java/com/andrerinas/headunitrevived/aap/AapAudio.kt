...
internal class AapAudio(
    ...
    private fun startAudioTrack(channel: Int) {
        ...
        // [FIX] AudioConfigs.get() only has entries for ID_AUD/ID_AU1/ID_AU2; this is the one
        // call site that passes a channel the phone chose (via an audio data message), not a
        // compile-time constant, so it's the one place that risked an NPE if that gate above it
        // (Channel.isAudio) ever loosened. Reject unknown channels instead of crashing.
        val config = AudioConfigs.get(channel)
        if (config == null) {
            AppLog.e("AapAudio.startAudioTrack: no AudioConfiguration for channel=$channel, ignoring")
            return
        }
