        ...
        when (message.type) {
            ...
            Media.MsgType.MEDIA_MESSAGE_VIDEO_FOCUS_REQUEST_VALUE -> {
                ...
                if (focusRequest.mode == Media.VideoFocusMode.VIDEO_FOCUS_NATIVE) {
                    // [FIX] VIDEO_FOCUS_NATIVE means "yield video focus to the head unit's
                    // native UI", which the phone also sends for transient handoffs (an
                    // incoming-call UI, an Assistant overlay, AA briefly backgrounded) — not
                    // only when the user taps Exit. Previously this tore the whole session down
                    // on the very first occurrence. Debounce: only treat it as a genuine exit
                    // once it recurs MAX_NATIVE_FOCUS_RETRIES times within
                    // NATIVE_FOCUS_DEBOUNCE_MS — a single transient handoff is ignored, a real
                    // exit (which keeps re-requesting native focus) still goes through.
                    val now = android.os.SystemClock.elapsedRealtime()
                    nativeFocusRequestCount = if (now - lastNativeFocusRequestTime > NATIVE_FOCUS_DEBOUNCE_MS) {
                        1
                    } else {
                    ...
                    if (nativeFocusRequestCount >= MAX_NATIVE_FOCUS_RETRIES) {
                        AppLog.i("Video Focus NATIVE received $nativeFocusRequestCount times within ${NATIVE_FOCUS_DEBOUNCE_MS}ms. Treating as genuine user exit. Stopping transport.")
                        aapTransport.wasUserExit = true
                        aapTransport.stop()
                        nativeFocusRequestCount = 0
                    } else {
                        AppLog.i("Video Focus NATIVE received ($nativeFocusRequestCount/$MAX_NATIVE_FOCUS_RETRIES within debounce window) — treating as a transient focus handoff, not exiting.")
                    }
                }
                return 0
            }
