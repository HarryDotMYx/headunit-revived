...
class AapMediaPlayback(
    ...
    private fun processMetadataPacket(message: AapMessage, flags: Int) {
        when (flags) {
            FLAG_FIRST_FRAGMENT -> {
                messageBuffer.clear()
                if (!tryPut(message.data, message.dataOffset, message.size - message.dataOffset)) return
                started = true
            }

            FLAG_MIDDLE_FRAGMENT -> {
                if (!started) return
                if (!tryPut(message.data, 0, message.size)) return
            }
    ...
    /**
     * Bounds-checked put into [messageBuffer].
     *
     * [FIX] `ByteBuffer.put()` throws `BufferOverflowException` if the payload exceeds the fixed
     * 1MB buffer (e.g. unusually large album art). That used to propagate straight out of
     * [processMetadataPacket] — critically, on the last fragment this put() ran *before* the
     * try/finally below, so `started` was never reset and the buffer was never cleared. Every
     * later fragment then hit the same already-full buffer and threw again, silently (no crash,
     * no obvious symptom — just dead media metadata). Returns `false` and resets state on
     * overflow so the caller can bail out cleanly instead of throwing.
     */
    private fun tryPut(data: ByteArray, offset: Int, length: Int): Boolean {
        if (length > messageBuffer.remaining()) {
            AppLog.w("AapMediaPlayback: metadata payload overflowed the ${messageBuffer.capacity()}-byte buffer (needed $length, had ${messageBuffer.remaining()} remaining) — dropping message")
            started = false
            messageBuffer.clear()
            return false
        }
