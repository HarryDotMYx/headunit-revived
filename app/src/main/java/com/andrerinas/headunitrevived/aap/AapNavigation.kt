...
class AapNavigation(
    ...
    // [FIX] Written on the main thread (debouncedBroadcastEmitter) and read/written on the AAP
    // poll thread (scheduleDebouncedBroadcast). Without @Volatile there's no guarantee the poll
    // thread ever observes the main thread's `= false` write — a stale cached `true` would make
    // scheduleDebouncedBroadcast's guard early-return forever, silently stopping turn-by-turn
    // broadcasts after the first one.
    @Volatile private var isBroadcastScheduled = false
    private var pendingNavEventType = NAV_EVENT_TYPE_TURN
    // [FIX] Guards `snapshot`: mutated on the poll thread inside process() below, read on the
    // main thread by debouncedBroadcastEmitter. Mutations from every process() call after the
    // first one in a debounce window don't accompany a fresh postDelayed() (scheduleDebounced
    // Broadcast's guard skips re-posting), so they had no happens-before edge to the main
    // thread's later read and could be observed stale/partially-updated. Sharing one lock
    // between every mutation site (via the synchronized block in process()) and every read
    // gives them all a consistent happens-before relationship.
    ...
    fun process(message: AapMessage): Boolean {
        ...
        return synchronized(navigationLock) { when (message.type) {
