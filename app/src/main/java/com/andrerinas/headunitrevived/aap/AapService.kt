...
class AapService : Service(), UsbReceiver.Listener {
    ...
    /**
     * True from the moment [onConnected] acquires session resources (WifiLock, permanent audio
     * focus, GpsLocationService, NightModeManager, car-key receivers) until [onDisconnected]
     * releases them. Distinct from [hasEverConnected] (which only flips true after a full
     * handshake): gating cleanup on hasEverConnected meant that if the *first* connection
     * attempt of the service's lifetime failed its handshake after onConnected() already
     * acquired everything, onDisconnected() was skipped entirely — leaking the high-perf
     * WifiLock, GPS tracking, and (in static-focus mode) permanent AUDIOFOCUS_GAIN indefinitely,
     * with the car radio staying muted even though AA never connected.
     */
    private var hasAcquiredConnectionResources = false
    ...
    /**
     * [FIX] checkAlreadyConnectedUsb() used to silently drop any USB check that arrived while
     * isSwitchingToAccessory was held — no log, no reschedule. UsbAccessoryMode.connectAndSwitch
     * sleeps 500ms after sending ACC_REQ_START waiting for the phone to re-enumerate as an
     * accessory, and isSwitchingToAccessory is held for that whole window. If the phone's
     * re-enumeration broadcast (a fresh onUsbAttach for the new accessory-mode device) lands
     * inside that window — a coin flip depending on how fast the phone re-enumerates — the event
     * that would have connected it was simply lost, with no automatic recovery until the user
     * manually replugged or hit the USB button. This flag lets checkAlreadyConnectedUsb queue a
     * recheck instead of dropping the event, run once launchAccessorySwitch's block finishes.
     */
    private val pendingUsbRecheck = AtomicBoolean(false)

    /**
     * Runs [block] (a USB accessory-switch/connect operation) on [scope] with
     * isSwitchingToAccessory already held — set synchronously by the caller before invoking this,
     * so a concurrent checkAlreadyConnectedUsb() call sees it immediately, before the coroutine
     * even gets a chance to start. Centralizes clearing the flag and honoring
     * [pendingUsbRecheck] so every call site gets the queued-recheck fix consistently, rather
     * than duplicating (and risking missing it at) each of the half-dozen places that used to
     * hand-roll this set/launch/finally pattern.
     */
    private fun launchAccessorySwitch(
        ...
        scope.launch(context) {
            try {
                ...
                if (pendingUsbRecheck.getAndSet(false)) {
                    AppLog.i("AapService: Re-running USB check after a pending recheck queued during the accessory switch.")
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            try {
                nearbyManager = NearbyManager(
                    this,
                    serviceScope,
                    onNewDeviceNeedsApproval = { endpointId, endpointName, authDigits ->
                        notifyNearbyDeviceNeedsApproval(endpointId, endpointName, authDigits)
                    }
                ) { socket ->
            ...
            commManager.connectionState.collect { state ->
                when (state) {
                    ...
                    is CommManager.ConnectionState.Disconnected -> {
                        // [FIX] Only check wasHandshakeFailure here — this used to ALSO call
                        // onHandshakeFailed() from an Error branch above, reasoning that it was
                        // "harmless to keep as a redundant path" since _connectionState is a
                        // conflating MutableStateFlow and a slow collector might miss the
                        // transient Error state entirely. That reasoning was incomplete: the
                        // collector doesn't *always* miss it — when it does happen to observe
                        // both Error and this Disconnected for the same failure, both branches
                        // fired and accessoryHandshakeFailures was incremented twice for one
                        // failure, making the MAX_STALE_ACCESSORY_RETRIES threshold trigger
                        // inconsistently (sometimes after 1 failure, sometimes after 2). This
                        // Disconnected/wasHandshakeFailure check is the one guaranteed-observed
                        // signal (see its declaration) — it alone is sufficient and correct.
                        if (state.wasHandshakeFailure) {
                            onHandshakeFailed()
                        }
                        // hasAcquiredConnectionResources (not hasEverConnected) gates cleanup:
                        // see its declaration for why. Still skips the spurious replayed
                        // Disconnected() that StateFlow delivers on startup, since onConnected()
                        // hasn't run yet at that point either.
                        if (hasAcquiredConnectionResources) {
                            hasAcquiredConnectionResources = false
    ...
    private fun onConnected() {
        isSwitchingToAccessory.set(false)
        hasAcquiredConnectionResources = true
        updateNotification()
        acquireWifiLock()
    ...
    /**
     * [FIX] Part of closing the "any LAN host can become the AAP peer" vulnerability: instead of
     * auto-connecting to a newly-discovered, never-approved host, prompt the user once. Tapping
     * fires [ACTION_APPROVE_DISCOVERED_HOST], which remembers the host (so it auto-connects next
     * time, same as before) and connects immediately.
     */
    private fun notifyDiscoveredHostNeedsApproval(ip: String, port: Int) {
        val notificationId = peerNotificationId(DISCOVERED_HOST_NOTIFICATION_ID_BASE, ip)
        val approveIntent = Intent(this, AapService::class.java).apply {
            action = ACTION_APPROVE_DISCOVERED_HOST
            putExtra(EXTRA_DISCOVERED_IP, ip)
            putExtra(EXTRA_DISCOVERED_PORT, port)
        }
        val piFlags = PendingIntent.FLAG_UPDATE_CURRENT or
            (if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) PendingIntent.FLAG_IMMUTABLE else 0)
        val approvePi = PendingIntent.getService(this, notificationId, approveIntent, piFlags)

        val notification = NotificationCompat.Builder(this, App.bootStartChannel)
            .setSmallIcon(R.drawable.ic_stat_aa)
            .setPriority(NotificationCompat.PRIORITY_HIGH)
            .setContentTitle(getString(R.string.discovered_host_needs_approval_title))
            .setContentText(getString(R.string.discovered_host_needs_approval_text, ip))
            .setContentIntent(approvePi)
            .setAutoCancel(true)
            .build()
    ...
    /**
     * [FIX] The AA Wireless BT RFCOMM channel is reachable by *any* Bluetooth-paired device
     * (it's a well-known UUID, not scoped to a "real" AA phone), and used to unconditionally
     * hand the WiFi Direct SSID/PSK to whatever connected. Prompt the user once per new MAC,
     * same pattern as [notifyDiscoveredHostNeedsApproval]. Tapping fires
     * [ACTION_APPROVE_BLUETOOTH_DEVICE], which adds the MAC to
     * [Settings.autoStartBluetoothDeviceMacs] so the phone's automatic retry succeeds.
     */
    fun notifyBluetoothDeviceNeedsApproval(mac: String, name: String) {
        val notificationId = peerNotificationId(BLUETOOTH_DEVICE_NOTIFICATION_ID_BASE, mac)
        val approveIntent = Intent(this, AapService::class.java).apply {
            action = ACTION_APPROVE_BLUETOOTH_DEVICE
            putExtra(EXTRA_BT_DEVICE_MAC, mac)
            putExtra(EXTRA_BT_DEVICE_NAME, name)
        }
        val piFlags = PendingIntent.FLAG_UPDATE_CURRENT or
            (if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) PendingIntent.FLAG_IMMUTABLE else 0)
        val approvePi = PendingIntent.getService(this, notificationId, approveIntent, piFlags)
        ...
        val notification = NotificationCompat.Builder(this, App.bootStartChannel)
            .setSmallIcon(R.drawable.ic_stat_aa)
            .setPriority(NotificationCompat.PRIORITY_HIGH)
            .setContentTitle(getString(R.string.bluetooth_device_needs_approval_title))
            .setContentText(getString(R.string.bluetooth_device_needs_approval_text, name))
            .setContentIntent(approvePi)
            .setAutoCancel(true)
            .build()
    ...
    /**
     * [FIX] Same pattern for Google Nearby Connections: NearbyManager no longer auto-accepts an
     * unrecognized endpoint. Tapping fires [ACTION_APPROVE_NEARBY_DEVICE], which tells
     * NearbyManager to remember the endpoint name and accept the still-pending session.
     *
     * [endpointName] is an attacker-controlled display string, not a verified device identity —
     * Nearby Connections has no stable identifier to offer instead. [authDigits] is the
     * short code Nearby Connections generates for this specific connection attempt so it can be
     * compared, by eye, against the same code shown on the other device; it's included in the
     * notification text so the approval isn't based on the name alone.
     */
    private fun notifyNearbyDeviceNeedsApproval(endpointId: String, endpointName: String, authDigits: String) {
        val notificationId = peerNotificationId(NEARBY_DEVICE_NOTIFICATION_ID_BASE, endpointId)
        val approveIntent = Intent(this, AapService::class.java).apply {
            action = ACTION_APPROVE_NEARBY_DEVICE
            putExtra(EXTRA_NEARBY_ENDPOINT_ID, endpointId)
            putExtra(EXTRA_NEARBY_ENDPOINT_NAME, endpointName)
        }
        val piFlags = PendingIntent.FLAG_UPDATE_CURRENT or
            (if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) PendingIntent.FLAG_IMMUTABLE else 0)
        val approvePi = PendingIntent.getService(this, notificationId, approveIntent, piFlags)
        ...
        val notification = NotificationCompat.Builder(this, App.bootStartChannel)
            .setSmallIcon(R.drawable.ic_stat_aa)
            .setPriority(NotificationCompat.PRIORITY_HIGH)
            .setContentTitle(getString(R.string.nearby_device_needs_approval_title))
            .setContentText(getString(R.string.nearby_device_needs_approval_text, endpointName, authDigits))
            .setStyle(NotificationCompat.BigTextStyle().bigText(getString(R.string.nearby_device_needs_approval_text, endpointName, authDigits)))
            .setContentIntent(approvePi)
            .setAutoCancel(true)
            .build()
    ...
    /**
     * [FIX] All three peer-approval notifications used one fixed ID + PendingIntent request
     * code for every peer of that type. Since PendingIntent matching ignores extras, a second
     * unapproved peer showing up before the first was handled would (with FLAG_UPDATE_CURRENT)
     * silently repoint the *same* on-screen notification's PendingIntent at the newer peer's
     * approve action — tapping what still visually reads as the first prompt could approve the
     * second peer instead — and would suppress the first peer's prompt entirely, letting a
     * repeatedly-connecting attacker keep a legitimate device's approval prompt from ever
     * showing. Give each peer identity its own notification/request-code slot. [base] values are
     * chosen far outside the small range of this app's other fixed notification IDs.
     */
    private fun peerNotificationId(base: Int, identity: String): Int {
        return base + (identity.hashCode() and 0x0FFFFFFF)
    }
    ...
    private fun scheduleReconnectIfNeeded(state: CommManager.ConnectionState.Disconnected) {
        ...
        // [FIX] This USB-specific retry used to be gated behind `if (wirelessServer != null)
        // { ...; return }` above it — but wirelessServer is non-null for the *entire* service
        // lifetime whenever wifiConnectionMode is 1, 2, or 3 (every mode except Manual), which
        // includes the default install (mode defaults to 2). So for virtually every user, a USB
        // handshake failure or unclean disconnect never reached this block at all: the wireless
        // discovery restart below ran instead, which has nothing to do with retrying USB. The
        // USB device would sit there — attached, in accessory mode, with permission, just not
        // connected — until some unrelated event (another physical attach, a manual retry)
        // happened to fix it. That's the mechanism behind "kadang okay kadang tidak, lama-lama
        // stable": whether it "just works" depends on whether one of those unrelated triggers
        // happens to fire, not on this retry (which never ran). Evaluate the USB path first,
        // independent of wirelessServer, so it can't be shadowed by the wireless path.
    ...
    private fun initWifiModeWithOptionalWait() {
        ...
        // [FIX] This used to gate the wait on mode 2/strategy 1 only, but initWifiMode() treats
        // mode 3 (Native AA) as needing WiFi Direct too (see usesWifiDirect below) — and mode 3
        // depends on it *more*, since it's the one that creates the P2P group directly rather
        // than waiting for a helper app. On a cold boot where the WiFi radio takes a few seconds
        // to come up, mode 3 attempted P2P group creation before WiFi was ready, failed, and
        // never retried until the user manually triggered a wireless scan.
        val usesWifiDirect = (settings.wifiConnectionMode == 3) ||
            (settings.wifiConnectionMode == 2 && settings.helperConnectionStrategy == 1)
        if (!usesWifiDirect || !settings.waitForWifiBeforeWifiDirect) {
            initWifiMode()
            return
        }
    ...
    override fun onDestroy() {
        ...
        // [FIX] onAudioFocusStateChanged captures `this` (via updateMediaSessionState) and was
        // never cleared here — only onDisconnected() cleared it, via releasePermanentAudioFocus
        // below. Destroying the service while still connected (force-stop, system kill, direct
        // stopSelf() outside the normal disconnect flow) skipped onDisconnected() entirely, so
        // this held a destroyed-Service reference for the life of the app-scoped CommManager,
        // and — more importantly — permanentFocusRequest (a *separate* AapService-level focus
        // request commManager.destroy()/AapAudio.releaseAllFocus() doesn't know about) stayed
        // held indefinitely: in static-focus mode the car radio would stay muted even though AA
        // never got a chance to hand focus back.
        ...
        // [FIX] onCreate() returns early (before setupCarMode() assigns this lateinit var) when
        // startForeground() throws — e.g. ForegroundServiceStartNotAllowedException on Android
        // 12+ when the service is started from the background (BootCompleteReceiver,
        // WifiAutoStartReceiver, or the onTaskRemoved restart below). onDestroy() still runs in
        // that case; an unguarded access here threw UninitializedPropertyAccessException and
        // crashed the process, skipping serviceScope.cancel() and super.onDestroy() below.
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (intent?.action) {
            ...
            ACTION_NATIVE_AA_POKE        -> {
                ...
                val settings = App.provide(this).settings
                if (mac != null && settings.wifiConnectionMode != 3) {
                    // [FIX] initWifiMode() always re-reads the persisted setting — it cannot be
                    // "forced" into mode 3 while the setting says otherwise. This branch used to
                    // call initWifiMode(force=true) unconditionally and then always fire
                    // manualPoke() below regardless of what mode ended up active: it would
                    // (re-)initialize whatever mode *is* configured (which stops
                    // nativeAaHandshakeManager for any non-mode-3 target) and then still wake
                    // the phone into looking for wireless AA over Bluetooth even though nothing
                    // was listening on the AA UUID — a failed wireless attempt on the phone with
                    // the head unit showing nothing.
                    AppLog.w("AapService: Native AA poke requested but wifiConnectionMode is ${settings.wifiConnectionMode}, not 3. Ignoring poke.")
                    ToastUtils.showToast(this, "Enable \"Native AA\" wireless mode in Settings before poking.", Toast.LENGTH_LONG)
                } else if (mac != null) {
            ...
            ACTION_APPROVE_DISCOVERED_HOST -> {
                // [FIX] Defense-in-depth: validate/normalize here too, at the point that
                // actually grants trust, not just at the deep-link entry points that currently
                // feed this action. A hostname must never enter approvedDiscoveredHosts — DNS
                // resolves at connect time, so its target could differ from what was shown.
                val ip = intent?.getStringExtra(EXTRA_DISCOVERED_IP)
                    ?.takeIf { it.isNumericIpAddress() }?.normalizedIpAddress()
                val port = intent?.getIntExtra(EXTRA_DISCOVERED_PORT, 5277) ?: 5277
                if (ip != null) {
                    AppLog.i("AapService: User approved discovered host $ip:$port. Remembering.")
                    val settings = App.provide(this).settings
                    settings.approvedDiscoveredHosts = settings.approvedDiscoveredHosts + ip
                    if (port == 5277) {
                        // We're the client for outbound discovery/deep-link connects — connect now.
                        serviceScope.launch { commManager.connect(ip, port) }
                    }
                    // For an inbound WirelessServer (port 5288) approval, the phone is the client
                    // and retries connecting on its own — nothing to proactively connect to here.
                    // The next attempt from this IP now passes WirelessServer's approval check.
                } else {
                    AppLog.w("AapService: ACTION_APPROVE_DISCOVERED_HOST with a non-numeric/missing host — ignoring.")
                }
            }
            ACTION_REQUEST_HOST_APPROVAL -> {
                val ip = intent?.getStringExtra(EXTRA_DISCOVERED_IP)
                    ?.takeIf { it.isNumericIpAddress() }?.normalizedIpAddress()
                val port = intent?.getIntExtra(EXTRA_DISCOVERED_PORT, 5277) ?: 5277
                if (ip != null) {
                    notifyDiscoveredHostNeedsApproval(ip, port)
                }
            }
            ACTION_APPROVE_BLUETOOTH_DEVICE -> {
                val mac = intent?.getStringExtra(EXTRA_BT_DEVICE_MAC)
                val name = intent?.getStringExtra(EXTRA_BT_DEVICE_NAME) ?: "Unknown Device"
                if (mac != null) {
                    AppLog.i("AapService: User approved Bluetooth device $mac ($name). Remembering.")
                    val settings = App.provide(this).settings
                    val newMacs = settings.autoStartBluetoothDeviceMacs + mac
                    settings.autoStartBluetoothDeviceMacs = newMacs
                    settings.autoStartBluetoothDeviceName = name
                    Settings.syncAutoStartBtMacsToDeviceStorage(this, newMacs)
                    // The phone retries the BT RFCOMM handshake on its own; the next attempt
                    // now passes NativeAaHandshakeManager's approval check.
                }
            }
            ACTION_APPROVE_NEARBY_DEVICE -> {
                val endpointId = intent?.getStringExtra(EXTRA_NEARBY_ENDPOINT_ID)
                val endpointName = intent?.getStringExtra(EXTRA_NEARBY_ENDPOINT_NAME)
                if (endpointId != null) {
                    AppLog.i("AapService: User approved Nearby device '$endpointName' ($endpointId).")
                    nearbyManager?.approvePendingConnection(endpointId)
                }
            }
    ...
    override fun onUsbPermission(granted: Boolean, connect: Boolean, device: UsbDevice) {
        ...
        if (granted) {
            ...
            if (UsbDeviceCompat.isInAccessoryMode(device)) {
                ...
                launchAccessorySwitch(serviceScope) {
                ...
                launchAccessorySwitch(serviceScope, Dispatchers.IO) {
    ...
    private fun checkAlreadyConnectedUsb(force: Boolean = false) {
        ...
        if (isSwitchingToAccessory.get()) {
            // [FIX] Don't just drop this — see pendingUsbRecheck's doc for why. Queue a recheck
            // for when the in-progress switch finishes instead of silently losing the event.
            AppLog.i("AapService: USB check arrived while an accessory switch is in progress. Queuing a recheck for when it finishes.")
            pendingUsbRecheck.set(true)
            return
        }
    ...
    private fun startDiscovery(oneShot: Boolean = false) {
        ...
        networkDiscovery = NetworkDiscovery(this, object : NetworkDiscovery.Listener {
            override fun onServiceFound(ip: String, port: Int, socket: java.net.Socket?) {
                ...
                when (port) {
                    5277 -> {
                        // [FIX] This used to connect to the FIRST responder on port 5277 on the
                        // LAN unconditionally — any host on the same Wi-Fi could stand up a fake
                        // AAP server and get an unauthenticated session (microphone, GPS, full
                        // screen) with zero user interaction. Only auto-connect to hosts the
                        // user has approved before; a new/unrecognized host gets a notification
                        // instead, and only connects once tapped (see ACTION_APPROVE_DISCOVERED_HOST).
                        if (App.provide(this@AapService).settings.approvedDiscoveredHosts.contains(ip)) {
                            AppLog.i("Auto-connecting to known Headunit Server at $ip:$port (reusing socket)")
                            serviceScope.launch {
                            ...
                            AppLog.w("Headunit Server found at $ip:$port but not yet approved — prompting user instead of auto-connecting.")
                            try { socket?.close() } catch (e: Exception) {}
                            notifyDiscoveredHostNeedsApproval(ip, 5277)
                        }
                    }
                    5289 -> {
    ...
    private inner class WirelessServer {
        ...
        fun start(registerNsd: Boolean = true) {
            ...
            job = serviceScope.launch(Dispatchers.IO) {
                try {
                    // [FIX] SO_REUSEADDR must be set *before* bind() to have any effect — the
                    // ServerSocket(port) constructor already binds immediately, so setting
                    // reuseAddress afterward was a no-op. Without it, rapidly stopping and
                    // restarting this server (e.g. initWifiMode()'s stop-then-start, or the
                    // mode-3 disconnect path's initWifiMode(force=true)) could hit the port
                    // still in TIME_WAIT and throw BindException.
                    ...
                    while (isActive) {
                        ...
                        serviceScope.launch {
                            if (commManager.isConnected) {
                                ...
                                // [FIX] This used to accept ANY client on this open port with no
                                // peer check at all — any host on the LAN could connect and get
                                // an unauthenticated AAP session (microphone, GPS, full screen).
                                // Same fix as NetworkDiscovery: only auto-accept a peer the user
                                // has approved before; a new/unrecognized peer gets a
                                // notification instead, and the connection is closed for now (AA
                                // Wireless clients retry on their own, and the retry will pass
                                // once approved).
                                val peerIp = clientSocket.inetAddress?.hostAddress
                                if (peerIp != null && App.provide(this@AapService).settings.approvedDiscoveredHosts.contains(peerIp)) {
                                    AppLog.i("WirelessServer: Accepted client connection from $peerIp. Passing to CommManager...")
                                    userExitedAA = false // Clear flag on genuine new connection
                                    commManager.connect(clientSocket)
                                } else {
                                    AppLog.w("WirelessServer: Connection from unapproved peer $peerIp — requiring approval instead of accepting directly.")
                                    withContext(Dispatchers.IO) {
                                        try { clientSocket.close() } catch (e: Exception) {}
                                    }
                                    if (peerIp != null) {
                                        notifyDiscoveredHostNeedsApproval(peerIp, 5288)
                                    }
                                }
                            }
                        }
                    ...
                    // [FIX] A bind()/accept() failure previously left the *outer* wirelessServer
                    // field pointing at this now-dead instance forever: startWirelessServer()'s
                    // `if (wirelessServer != null) return` guard silently no-op'd every future
                    // call, permanently disabling wireless connections until the service
                    // restarted. Self-heal by clearing the reference — but only if we're still
                    // the currently-active instance, since an intentional stopWirelessServer()
                    // may have already replaced us with a new one by the time this runs.
                    if (wirelessServer === this@WirelessServer) {
                        wirelessServer = null
                    }
                }
            }
        ...
        private fun registerNsd() {
            ...
            registrationListener = object : NsdManager.RegistrationListener {
                ...
                override fun onRegistrationFailed(info: NsdServiceInfo, err: Int) {
                    // [FIX] The framework has already released this listener internally when
                    // registration fails — holding onto it made unregisterNsd() call
                    // unregisterService() on a listener the framework doesn't recognize,
                    // throwing IllegalArgumentException. That used to happen with no try/catch
                    // inside start()'s `finally` block, and an exception thrown from a finally
                    // block isn't caught by that same try's own catch clause — so on any head
                    // unit where mDNS registration fails, the next stopWirelessServer() crashed
                    // the process from an unhandled coroutine exception.
                    AppLog.e("NSD Reg Fail: $err")
        ...
        private fun unregisterNsd() {
            // [FIX] Belt-and-suspenders alongside the onRegistrationFailed fix above: guard the
            // call itself too, in case unregisterService() throws for any other reason (e.g.
            // called twice, or the service died between registration and unregistration).
            try {
                registrationListener?.let { nsdManager?.unregisterService(it) }
            } catch (e: Exception) {
                AppLog.w("WirelessServer: unregisterService failed: ${e.message}")
            }
            registrationListener = null
        }
    ...
    companion object {
        ...
        /** Sent when the user taps the "New device found" notification to approve a
         *  network-discovered host that wasn't already in [Settings.approvedDiscoveredHosts]. */
        const val ACTION_APPROVE_DISCOVERED_HOST   = "com.andrerinas.headunitrevived.ACTION_APPROVE_DISCOVERED_HOST"
        /** Sent by MainActivity/AutomationActivity's `headunit://connect?ip=` deep link when the
         *  requested IP isn't already approved — shows the same approval notification instead of
         *  connecting directly, so a zero-permission app can't use the deep link to make this
         *  service connect to an attacker-chosen (e.g. loopback) AAP server. */
        const val ACTION_REQUEST_HOST_APPROVAL     = "com.andrerinas.headunitrevived.ACTION_REQUEST_HOST_APPROVAL"
        const val EXTRA_DISCOVERED_IP = "extra_discovered_ip"
        const val EXTRA_DISCOVERED_PORT = "extra_discovered_port"
        // Base offsets for peerNotificationId() — see its doc for why these are per-peer,
        // not fixed IDs, and why the bases are far outside this file's other small notification
        // IDs (e.g. BOOT_START_NOTIFICATION_ID=42, PROJECTION_LAUNCH_NOTIFICATION_ID=43).
        private const val DISCOVERED_HOST_NOTIFICATION_ID_BASE = 100_000

        /** Sent when the user taps the "New Bluetooth device" notification to approve a
         *  phone that performed the AA Wireless BT handshake but wasn't already in
         *  [Settings.autoStartBluetoothDeviceMacs]. Until approved, [NativeAaHandshakeManager]
         *  will not release the WiFi Direct SSID/PSK to it. */
        const val ACTION_APPROVE_BLUETOOTH_DEVICE = "com.andrerinas.headunitrevived.ACTION_APPROVE_BLUETOOTH_DEVICE"
        const val EXTRA_BT_DEVICE_MAC = "extra_bt_device_mac"
        const val EXTRA_BT_DEVICE_NAME = "extra_bt_device_name"
        private const val BLUETOOTH_DEVICE_NOTIFICATION_ID_BASE = 200_000

        /** Sent when the user taps the "New Nearby device" notification to approve an endpoint
         *  NearbyManager hasn't seen before (see [Settings.approvedNearbyDeviceNames]). */
        const val ACTION_APPROVE_NEARBY_DEVICE = "com.andrerinas.headunitrevived.ACTION_APPROVE_NEARBY_DEVICE"
        const val EXTRA_NEARBY_ENDPOINT_ID = "extra_nearby_endpoint_id"
        const val EXTRA_NEARBY_ENDPOINT_NAME = "extra_nearby_endpoint_name"
        private const val NEARBY_DEVICE_NOTIFICATION_ID_BASE = 300_000
