...
class AapTransport(
    ...
    private var lastFocusCycleRecoveryMs = 0L

    /**
     * [FIX] This had no rate limit at all. AapVideo's own corruption-recovery path already
     * rate-limits its keyframe requests to 1/s (see markCorruptAndRequestRecovery there), but
     * this one — also reachable from the decoder's stall-restart path — could re-fire roughly
     * every 3s indefinitely. Each cycle makes the phone send a fresh SPS/PPS + full I-frame: a
     * 100-300KB burst onto a link that may already be congested (e.g. 2.4GHz Wi-Fi sharing
     * spectrum with Bluetooth), which is exactly the condition most likely to trigger this in
     * the first place — stall -> recovery -> keyframe burst -> more congestion -> stall. Rate
     * limiting gives the link a chance to actually recover between cycles instead of being kept
     * saturated by the recovery mechanism itself.
     */
    private fun triggerFocusCycleRecovery() {
        val now = SystemClock.elapsedRealtime()
        if (now - lastFocusCycleRecoveryMs < FOCUS_CYCLE_RECOVERY_MIN_INTERVAL_MS) {
            AppLog.d("AapTransport: Skipping focus cycle recovery — last one was ${now - lastFocusCycleRecoveryMs}ms ago.")
            return
        }
        lastFocusCycleRecoveryMs = now
    ...
    companion object {
        ...
        // Minimum gap between triggerFocusCycleRecovery() calls. See its doc for why.
        private const val FOCUS_CYCLE_RECOVERY_MIN_INTERVAL_MS = 4000L
    }
}
