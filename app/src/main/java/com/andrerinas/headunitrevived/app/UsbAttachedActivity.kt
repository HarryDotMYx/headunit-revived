...
class UsbAttachedActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // [FIX] After connectAndSwitch() succeeds below, the phone re-enumerates as a *new* USB
        // device in accessory mode, which normally reaches AapService via a second
        // USB_DEVICE_ATTACHED broadcast (caught by this same Activity's manifest filter, which
        // then starts the service). On the exact hardware this whole flow already has to work
        // around — see the comment in AapService.onUsbAttach() about some Chinese MediaTek head
        // units not reliably delivering USB_DEVICE_ATTACHED to activities — that *second*
        // broadcast can be dropped just as easily as the first, silently leaving the phone
        // switched to accessory mode with nothing left alive to notice and connect to it. Start
        // AapService now, before attempting the switch, so its own device-list scan
        // (checkAlreadyConnectedUsb) and its dynamically-registered USB receiver are both
        // already live well before the re-enumeration happens — independent of whether that
        // second broadcast ever arrives. Harmless if the service is already running.
        if (!isLocked) {
            ContextCompat.startForegroundService(this, Intent(this, AapService::class.java).apply {
                action = AapService.ACTION_CHECK_USB
            })
        ...
        // [FIX] Was `settings?.useLibusb ?: false` — settings is null while the device is
        // locked (see isLocked above), so any USB attach during locked boot silently used the
        // non-libusb switch path regardless of what the user actually configured. Fall back to
        // the device-protected-storage mirror, which (unlike credential-storage prefs) is
        // readable before unlock.
        val useLibusb = settings?.useLibusb ?: Settings.isUseLibusbEnabled(this)
        Thread {
