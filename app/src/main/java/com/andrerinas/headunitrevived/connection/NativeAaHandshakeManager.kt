...
class NativeAaHandshakeManager(
    ...
    // @Volatile: written from start()/stop() and read from the IO coroutines' accept/poke loops.
    @Volatile private var isRunning = false
    // [FIX] Accepted sockets weren't tracked, so stop() only closed the two *server* sockets.
    // handleHandshake()'s readProtobuf() does an untimed DataInputStream.readFully() while
    // waiting for the phone's response — if the phone connects and then sends nothing (or dies
    // after the Type-1 send), that call blocks forever holding an RFCOMM channel and an IO
    // dispatcher thread hostage, even past stop(), since isRunning is only checked between
    // blocking calls. Track accepted sockets and close them in stop() so an in-flight blocking
    // read is forced to throw and unwind through the existing try/finally cleanup.
    private val acceptedSockets = Collections.synchronizedSet(mutableSetOf<BluetoothSocket>())
    ...
    fun start() {
        ...
        // [FIX] Only latch isRunning once we know we're actually starting the servers below.
        // This used to be set before the adapter check, so if Bluetooth was off when start()
        // was called, the return above left isRunning stuck true forever — every later start()
        // call (e.g. after the user turns Bluetooth on) then no-op'd via the `if (isRunning)
        // return` guard at the top, permanently disabling the Native AA handshake server for
    ...
    private suspend fun handleHandshake(socket: BluetoothSocket) = withContext(Dispatchers.IO) {
        try {
            ...
            // [FIX] This RFCOMM channel uses a well-known UUID reachable by any Bluetooth-paired
            // device, not just a genuine AA phone — it previously proceeded unconditionally,
            // handing the live WiFi Direct SSID/PSK to whoever connected and silently adding
            // them to the auto-start allowlist. Require one-time user approval for a new MAC
            // before releasing credentials; already-approved devices are unaffected.
            val macs = settings.autoStartBluetoothDeviceMacs
            if (!macs.contains(device.address)) {
                AppLog.w("NativeAA: New Bluetooth device ${device.address} (${device.name}) attempted the AA handshake. Requesting user approval before releasing WiFi credentials.")
                context.notifyBluetoothDeviceNeedsApproval(device.address, device.name ?: "Unknown Device")
                try { socket.close() } catch (_: Exception) {}
                return@withContext
            }
    ...
    fun stop() {
        ...
        // Force-close any already-accepted sockets too, so a handshake blocked in an untimed
        // readFully() (waiting for a phone that connected and then went silent) is unblocked
        // via IOException instead of holding its RFCOMM channel and IO thread forever.
        synchronized(acceptedSockets) {
            acceptedSockets.forEach { try { it.close() } catch (e: Exception) {} }
