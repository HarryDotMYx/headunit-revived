...
class UsbAccessoryConnection(private val usbMgr: UsbManager, private val device: UsbDevice) : AccessoryConnection {
    ...
    fun isDeviceRunning(device: UsbDevice): Boolean {
        // [FIX] This is called from the main thread (AapService.onUsbDetach ->
        // CommManager.isConnectedToUsbDevice). sStateLock is also held by connect()/usbOpen()
        // across up to two 1000ms Thread.sleep() retries while waiting for USB permission —
        // taking the same lock here could block the main thread for ~2s. usbDeviceConnected is
        // @Volatile and UsbDevice's vendor/product/manufacturer/product-name fields are static
        // descriptor info that doesn't mutate, so this read is safe without the lock.
    ...
    private fun usbOpen(device: UsbDevice) {
        ...
        // [FIX] Removed a dead "wait for permission" branch that never actually requested
        // permission (just logged and waited via the sleep below). It couldn't do anything
        // useful anyway: CommManager.connect(UsbDevice) already hard-fails before this class is
        // even constructed if permission isn't granted, so by the time usbOpen() runs, permission
        // is guaranteed. Keep only the openDevice()-returned-null retry, which is the one
        // legitimate transient failure this loop can actually help with.
        for (i in 0 until 3) {
            try {
        ...
        return try {
            ...
            while (totalReturned < length) {
                ...
                if (read < 0) {
                    ...
                    // [FIX] A readFully caller expects exactly `length` bytes back, -1 (fatal),
                    // or 0 (nothing consumed, safe to retry) — never a partial count that's
                    // less than requested. Returning totalReturned here used to silently hand
                    // the caller fewer bytes than it asked for while looking like success,
                    // permanently desynchronizing the AAP frame stream from then on (same class
                    // of bug as the analogous fix in SocketAccessoryConnection.recvBlocking).
                    // Non-readFully callers are unaffected: a partial count is a normal,
                    // expected outcome for them.
                    if (readFully && totalReturned > 0) {
                        AppLog.w("USB Read Error: readFully requested $length bytes but only got $totalReturned before a read error — reporting fatal instead of a silent partial read")
                        return -1
                    }
