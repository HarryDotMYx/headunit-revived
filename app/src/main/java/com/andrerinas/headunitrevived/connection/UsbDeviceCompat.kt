...
class UsbDeviceCompat(val wrappedDevice: UsbDevice) {
    ...
    companion object {
        ...
        fun isAndroidDevice(device: UsbDevice): Boolean {
            ...
            // [FIX] A phone plugged in "Charging only" / "No data transfer" mode (common when
            // it's locked) exposes no MTP/PTP/ADB/RNDIS interface, so hasAndroidInterface()
            // below finds nothing and the device was rejected here entirely — before it ever
            // got a chance to try the AOA switch. AOA vendor requests (GET_PROTOCOL/SEND_STRING/
            // ACC_REQ_START in UsbAccessoryMode) are device-level control transfers that work
            // regardless of which interfaces are currently exposed, so a Google-VID device
            // should be attempted rather than dismissed as "not Android" just because it's
            // charging-only right now. This was another "first attach ignored, works once the
            // phone re-enumerates with an interface (unlocked, USB mode changed)" path.
            if (device.vendorId == USB_VID_GOO) return true
