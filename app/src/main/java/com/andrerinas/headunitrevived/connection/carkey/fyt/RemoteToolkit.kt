        ...
        override fun onTransact(code: Int, data: Parcel, reply: Parcel?, flags: Int): Boolean {
            when (code) {
                ...
                TRANSACTION_PROC_NAME -> {
                    ...
                    reply!!.writeNoException()
                    // [FIX] Was writing to `data` (the incoming request parcel, discarded after
                    // this returns) instead of `reply` (what Proxy.procName()'s reply.readString()
                    // actually reads back). Currently unreachable — nothing in this codebase
                    // subclasses RemoteToolkit.Stub to serve this interface, only Proxy is used
                    // — but fixed so it can't become a live bug if that ever changes.
                    reply.writeString(strProcName)
                    return true
