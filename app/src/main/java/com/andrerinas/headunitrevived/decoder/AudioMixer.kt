...
class AudioMixer(
    ...
    companion object {
        ...
        // Cycles of buffered audio required to exit pre-rolling (jitter cushion), and the
        // level a channel's buffer is trimmed back down to once it does. See mixLoop().
        private const val PREROLL_CYCLES = 3
    }
    ...
    // [FIX] Was a local val in start() — never stored, so stop() had nothing to release,
    // leaking one native AudioEffect handle (bound to a now-dead audio session) per mixer
    // start/stop cycle.
    ...
    fun start() {
        ...
        try {
            ...
            // [FIX] running was never set true here, but audioTrack (and equalizer) could
            // already be non-null if the failure happened at track.play()/mixThread.start() —
            // release them instead of leaking a native AudioTrack/effect handle. Just as
            // important: AudioDecoder.start() used to only check `mixer == null` before
            // deciding whether to create+start a mixer, so a failure here left this same
            // half-dead instance in place for the rest of the session — every later channel
            // start would feed a mixer with no thread draining it, going permanently silent.
            // See the isRunning() check now guarding that call site.
    ...
    fun stop() {
        ...
        try {
            ...
            AppLog.e("$TAG: Error releasing Equalizer: ${e.message}")
        }
        equalizer = null
    ...
    // [FIX] AudioTrackWrapper.stop() is asynchronous (interrupt + async cleanup on the
    // wrapper's own thread), while AudioDecoder immediately creates+registers a *new* wrapper
    // for the same channel number on a quick stop-then-restart (e.g. a pause/resume or nav-duck
    // cycle). If the old wrapper's cleanup() ran unregisterChannel() after the new one had
    // already registered, it would wipe out the new registration instead of its own — going
    // permanently, silently silent on that channel with no log output. Each registration gets a
    // generation stamp; unregister only takes effect if the caller still holds the current one.
    private val channelGeneration = ConcurrentHashMap<Int, Long>()
    private val generationCounter = java.util.concurrent.atomic.AtomicLong(0)

    /**
     * Register a channel with its audio parameters. Returns a generation token that must be
     * passed back to [unregisterChannel] so a stale (already-superseded) caller can't clear a
     * newer registration for the same channel number.
     */
    fun registerChannel(channel: Int, sampleRate: Int, channelCount: Int): Long {
        ...
        AppLog.i("$TAG: Registered channel $channel (sampleRate=$sampleRate, channels=$channelCount, gen=$generation)")
        return generation
    }

    /**
     * Unregister a channel (e.g. on session end). No-ops if [generation] no longer matches the
     * channel's current registration (a newer wrapper has already taken over this channel).
     */
    fun unregisterChannel(channel: Int, generation: Long) {
        if (channelGeneration[channel] != generation) {
            AppLog.i("$TAG: Skipping unregister of channel $channel (gen=$generation) — superseded by a newer registration")
            return
        }
        channelGeneration.remove(channel)
    ...
    fun feed(channel: Int, data: ByteArray, offset: Int, length: Int) {
        ...
        if (config.sampleRate == OUTPUT_SAMPLE_RATE && config.channelCount == OUTPUT_CHANNELS) {
            ...
            writeToBuffer(channel, buffer, inputShorts, inputShortsSize)
            ...
            writeToBuffer(channel, buffer, outputShorts, outputShortsSize)
        } else {
    ...
    // [FIX] ShortCircularBuffer.write() silently stops (and returns a short count) once full;
    // feed()'s three call sites never checked that, so a sustained overflow (e.g. the mixer
    // thread stalled, or a channel stuck pre-rolling for a long stretch) dropped audio with
    // zero log output anywhere.
    private fun writeToBuffer(channel: Int, buffer: ShortCircularBuffer, data: ShortArray, length: Int) {
        val written = buffer.write(data, length)
        if (written < length) {
            AppLog.w(TAG, "Channel $channel: buffer full, dropped ${length - written} samples")
        }
    }
        ...
        while (running.get()) {
            ...
            for ((channel, buffer) in channelBuffers) {
                ...
                if (isPreRolling) {
                    ...
                    if (buffer.size() >= FRAMES_PER_CYCLE * PREROLL_CYCLES) {
                        channelPreRolling[channel] = false
                        isPreRolling = false
                        // [FIX] feed() keeps writing incoming audio the whole time a channel is
                        // pre-rolling (e.g. after an underrun), so a long stall let the buffer
                        // accumulate far more than the jitter cushion this threshold intends —
                        // and nothing ever drained the excess back out, so every underrun baked
                        // in permanent extra latency that only ever grew over a session. Trim
                        // back down to the cushion size instead of playing through the backlog.
                        val excess = buffer.size() - FRAMES_PER_CYCLE * PREROLL_CYCLES
                        if (excess > 0) {
                            val dropped = buffer.skip(excess)
                            AppLog.w(TAG, "Channel $channel: dropping ${dropped} stale samples after underrun to reclaim latency")
                        }
                    } else {
    ...
    private class ShortCircularBuffer(capacity: Int) {
        ...
        /** Discards up to [length] samples from the front without copying them out. Returns
         *  the number actually discarded. */
        @Synchronized
        fun skip(length: Int): Int {
            val toSkip = length.coerceAtMost(size)
            head = (head + toSkip) % buffer.size
            size -= toSkip
            return toSkip
        }
