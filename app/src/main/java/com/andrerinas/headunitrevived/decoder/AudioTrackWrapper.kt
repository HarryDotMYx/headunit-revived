...
class AudioTrackWrapper(
    ...
    companion object {
        ...
        // See maxUnboundedQueueBytes's doc for why this exists.
        private const val MAX_UNBOUNDED_QUEUE_BACKLOG_MS = 150
    }
    ...
    // Generation token returned by AudioMixer.registerChannel(); see cleanup()'s unregister call.
    private var mixerGeneration: Long = -1
    ...
    // [FIX] audioQueueCapacity defaults to 0 ("Unbounded (Legacy)" in Quick Settings), which
    // made dataQueue's overflow-drop branch in write() unreachable in practice: every network
    // hiccup (e.g. a 2.4GHz Wi-Fi + Bluetooth coexistence stall) let TCP/OS buffers absorb the
    // gap and deliver a burst on recovery; offer() accepted the whole burst instantly, and the
    // playback thread (run(), below) only drains at 1x real time — so the delay never recovered
    // and kept compounding across the drive. This tracks the queued backlog by DURATION (not
    // chunk count, which says nothing about latency) so write() can drop the OLDEST buffered
    // audio on overflow instead of leaving the backlog to grow unbounded. Only engages when
    // audioQueueCapacity <= 0; an explicit positive value keeps its original chunk-count-bounded
    // behavior unchanged.
    private val bytesPerMs: Int = ((sampleRateInHz.toLong() * bytesPerFrame) / 1000).toInt().coerceAtLeast(1)
    private val maxUnboundedQueueBytes: Int = MAX_UNBOUNDED_QUEUE_BACKLOG_MS * bytesPerMs
    ...
    init {
        ...
        if (mixer != null) {
            mixerGeneration = mixer.registerChannel(channelId, sampleRateInHz, channelCount)
            mixer.setChannelGain(channelId, gain)
        } else {
            setVolume(gain)
    ...
    private fun writeToTrack(buffer: ByteArray, size: Int) {
        if (mixer != null) {
            ...
            if (result > 0) {
                ...
                // [FIX] A negative result is a real AudioTrack error (e.g. ERROR_DEAD_OBJECT if
                // the head unit's audio HAL reroutes/kills the track, ERROR_INVALID_OPERATION) —
                // not just "wrote 0 bytes". This was silently swallowed, so audio could die
                // completely with zero diagnostics to explain why.
                AppLog.e("AudioTrack.write() failed with error code $result")
            }
        }
    }
    ...
    fun write(buffer: ByteArray, offset: Int, size: Int) {
        ...
        try {
            ...
            val chunk = AudioChunk(data, size)
            if (audioQueueCapacity > 0) {
                // Explicit legacy chunk-count bound — behavior unchanged.
                val success = dataQueue.offer(chunk, 5, TimeUnit.MILLISECONDS)
                if (!success) {
                ...
                // [FIX] See maxUnboundedQueueBytes's doc. put() cannot fail (queue is
                // unbounded), so instead of ever rejecting the newest frame, drop the OLDEST
                // buffered audio until we're back under the target backlog — catching up to
                // live instead of accumulating latency forever.
                dataQueue.put(chunk)
                var overflow = queuedBytes.addAndGet(size) - maxUnboundedQueueBytes
                while (overflow > 0) {
                    val dropped = dataQueue.poll() ?: break
                    queuedBytes.addAndGet(-dropped.size)
                    overflow -= dropped.size
                    recycleAudioBuffer(dropped.data)
                }
            }
        } catch (e: InterruptedException) {
    ...
    private fun cleanup() {
        ...
        if (mixer != null) {
            mixer.unregisterChannel(channelId, mixerGeneration)
        }
        ...
        // [FIX] equalizer was assigned by attachHwDspEqualizerQuietly() but never released here,
        // leaking one native AudioEffect handle (bound to a now-dead audio session) per channel
        // stop/start cycle when HW DSP equalizer attachment is enabled.
        try {
            equalizer?.release()
        } catch (e: Exception) {
