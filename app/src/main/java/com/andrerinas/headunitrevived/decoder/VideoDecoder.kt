...
class VideoDecoder(private val settings: Settings) {
    companion object {
        ...
        // See feedInputBuffer's doc for why this is capped low (was 30).
        private const val MAX_DEQUEUE_INPUT_ATTEMPTS = 3
    ...
    // [FIX] Split from the old single codecConfigured flag, which conflated two different
    // meanings: "have we parsed the SPS/PPS/VPS we need" and "is the codec currently started".
    // hasParsedRequiredNals tracks the former; codecConfigured (below) now tracks only the
    // latter. See start() and stop() for why this matters.
    private var hasParsedRequiredNals = false
    ...
    fun stop(reason: String = "unknown") {
        synchronized(this) {
            running = false
            var outputThreadStillAlive = false
            try {
                ...
                if (outputThread != null && outputThread != Thread.currentThread()) {
                    ...
                    outputThreadStillAlive = outputThread?.isAlive == true
                }
            } catch (e: Exception) {}
            outputThread = null

            if (outputThreadStillAlive) {
                // [FIX] The output thread can be blocked inside a native MediaCodec call
                // (dequeueOutputBuffer/releaseOutputBuffer) on a stalled Surface consumer —
                // exactly the condition the issue #650 display-stall recovery detects and
                // reacts to by calling this stop(). interrupt() cannot forcibly abort a thread
                // blocked in native code, so a timed-out join() here used to be ignored:
                // codec.release() ran anyway, freeing native memory a JNI call on the other
                // thread might still be using (use-after-free, SIGSEGV, uncatchable from here).
                // Leak the reference instead of crashing — MediaCodec has a finalizer that
                // releases it eventually, and this VideoDecoder instance won't touch it again
                // either way since the fields are cleared below.
                AppLog.e("VideoDecoder: output thread did not stop in time (reason=$reason) — leaking codec reference to avoid a native use-after-free crash")
            } else {
                try {
                    codec?.stop()
                } catch (e: Exception) {}
    ...
    private fun scanAndApplyConfig(buffer: ByteArray, offset: Int, size: Int, type: CodecType) {
        forEachNalUnit(buffer, offset, size) { nalData, headerLen ->
            ...
            if (type == CodecType.H264) {
                ...
                if (sps != null) hasParsedRequiredNals = true
            } else {
                ...
                if (vps != null && sps != null) hasParsedRequiredNals = true
            }
        }
    }
    ...
    private fun start(mimeType: String, forceSoftware: Boolean, width: Int, height: Int) {
        try {
            ...
            codec?.start()
            // [FIX] This used to only ever be set inside scanAndApplyConfig (now
            // hasParsedRequiredNals), so after a restart — which keeps the cached vps/sps/pps
            // but resets this flag — it stayed false forever unless a *later* packet happened to
            // be scanned and matched a parameter-set NAL again. Meanwhile feedInputBuffer's
            // `shouldAlwaysFlagConfig() || !codecConfigured` check kept flagging
            // BUFFER_FLAG_CODEC_CONFIG on every packet that merely started with a parameter-set
            // NAL, discarding any slice data bundled in the same access unit (a common keyframe
            // encoding: SPS+PPS+IDR in one AU). We just successfully configured and started the
            // codec using the cached CSD above, so it genuinely is configured now.
    ...
    private fun feedInputBuffer(buffer: ByteBuffer): Boolean {
        ...
        try {
            var inputIndex = -1
            var attempts = 0
            // [FIX] This used to retry up to 30 times at 10ms each (up to 300ms) while holding
            // the `synchronized(this)` monitor decode() takes — and decode() runs on the network
            // read thread. During that whole window the socket isn't read at all, which makes an
            // already-struggling link (e.g. 2.4GHz Wi-Fi sharing spectrum with Bluetooth) worse:
            // the phone's send buffer/TCP window backs up further, and other messages (audio)
            // queued behind this one in the same stream arrive later and in a bigger burst. Cap
            // the budget much lower and, on failure, request a fresh keyframe instead of
            // silently stalling the read thread.
            while (attempts < MAX_DEQUEUE_INPUT_ATTEMPTS) {
            ...
            if (inputIndex < 0) {
                AppLog.e("Input buffer feed failed (full) after $attempts attempts — requesting keyframe recovery")
                onDecoderError?.invoke()
                return false
            }
            ...
            if (buffer.remaining() <= capacity) {
                ...
                // [FIX] This used to "truncate" by copying only the first `capacity` bytes into
                // this input buffer, then restoring buffer's original limit so decode()'s
                // `while (buf.hasRemaining())` loop fed the remainder as a *second*, separate
                // queueInputBuffer() call — different PTS, no BUFFER_FLAG_PARTIAL_FRAME — so
                // MediaCodec saw two bogus access units instead of one real one: an incomplete
                // NAL followed by a fragment with no valid start code. That reliably produces a
                // decode error, and large keyframes are exactly what's most likely to exceed
                // KEY_MAX_INPUT_SIZE (1-2MB depending on API level/resolution) — so this could
                // repeat every keyframe: decode error -> consecutiveErrors -> restart -> next
                // large keyframe -> repeat. Drop the whole oversized access unit cleanly instead
                // of feeding it in mangled pieces, and request a keyframe so the stream can
                // recover from a clean state.
                AppLog.w("Frame too large: ${buffer.remaining()} > $capacity. Dropping this access unit and requesting a keyframe.")
    ...
    private fun outputThreadLoop() {
        ...
        while (running) {
            ...
            try {
                ...
                // [FIX] This used to fire on output silence alone, unable to tell "decoder is
                // wedged" apart from "network delivered nothing" (e.g. a 2.4GHz Wi-Fi + BT
                // coexistence stall). A false positive here tears the codec down and, via
                // onDecoderError -> triggerFocusCycleRecovery, makes the phone send a fresh
                // SPS/PPS + full I-frame — a burst onto a link that's already struggling, making
                // the congestion (and the user-visible lag) worse, not better. lastInputBytesReceivedMs
                // (set in decode(), on every packet received) already exists specifically to make
                // this distinction (see its declaration, issue #650) — only restart when input is
                // also actively flowing, meaning the decoder itself is genuinely stuck rather than
                // just starved.
                if (lastOutputMs > 0) {
                    val stallGap = SystemClock.elapsedRealtime() - lastOutputMs
                    if (stallGap > 3000L) {
                        val inputGap = SystemClock.elapsedRealtime() - lastInputBytesReceivedMs
                        if (lastInputBytesReceivedMs > 0 && inputGap < 3000L) {
                            AppLog.w("Decoder stall detected (no output for ${stallGap}ms, input still flowing ${inputGap}ms ago). Forcing restart.")
                            scheduleRestart("sync_stall")
                            break
                        } else {
                            AppLog.d("No decoder output for ${stallGap}ms, but input has also been quiet for ${inputGap}ms — likely a network/phone pause, not a decoder stall. Not restarting.")
                        }
                    }
                }
            } catch (e: Exception) {
