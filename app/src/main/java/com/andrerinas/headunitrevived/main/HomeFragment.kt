...
class HomeFragment : Fragment() {
        ...
        return when (connectionType) {
            Settings.CONNECTION_TYPE_WIFI -> {
                if (appSettings.wifiConnectionMode == 1) {
                    ...
                    // [FIX] Defense-in-depth: lastConnectionIp is only ever meant to hold a host
                    // that already went through the approvedDiscoveredHosts gate (see
                    // AapService/NetworkListFragment discovery) before a successful connect ever
                    // saved it here. Re-checking at this auto-reconnect-on-launch site means a
                    // future regression in one of those write paths can't silently turn into an
                    // unattended reconnect to an unapproved host.
                    if (ip.isNotEmpty() && appSettings.approvedDiscoveredHosts.contains(ip)) {
    ...
    private fun showNearbyDeviceSelector() {
        ...
        deviceListView.setOnItemClickListener { _, _, which, _ ->
            // [FIX] Used to re-read NearbyManager.discoveredEndpoints.value fresh here instead
            // of asking the adapter for the item at this position. The list live-updates on a
            // background collector (below) as endpoints are found/lost — if that StateFlow
            // emitted a new list in the gap between the tap and this callback running, `which`
            // could resolve to a different endpoint in the fresh list than the one actually
            // rendered at that row when the user tapped it. Read from the adapter's own backing
            // data instead, which is guaranteed to match what was on screen.
            val endpoint = listAdapter.getItem(which)
            if (endpoint != null) {
