...
class KeymapFragment : Fragment(), MainActivity.KeyListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // [FIX] Used to be wired in onAttach(), which runs before this method — a physical key
        // event dispatched to MainActivity.keyListener in that window would call onKeyEvent()
        // above, which unconditionally writes to keypressDebuggerTextView (a lateinit view field
        // not yet assigned), crashing with UninitializedPropertyAccessException. Wire it here,
        // after every lateinit field this listener touches is guaranteed to be initialized.
        (activity as? MainActivity)?.keyListener = this
    }
    ...
    override fun onResume() {
        super.onResume()
        // [FIX] IntentFilters.keyEvent is a shared, process-wide singleton — calling
        // addAction() directly on it mutated it permanently for every other registration too
        // (e.g. AapProjectionActivity's keyEventReceiver would start matching DEBUG_KEY as well
        // once this screen had been visited once this process). Copy it instead of mutating
        // the shared instance.
        val filter = IntentFilter(IntentFilters.keyEvent)
    ...
    override fun onAttach(context: Context) {
        ...
        // keyListener is wired later, in onViewCreated() — see the [FIX] note there.
        (activity as? MainActivity)?.setDefaultKeyMode(Activity.DEFAULT_KEYS_DISABLE)
    }
