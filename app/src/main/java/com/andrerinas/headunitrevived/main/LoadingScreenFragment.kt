        ...
        viewLifecycleOwner.lifecycleScope.launch {
            val outcome = withContext(Dispatchers.IO) {
                try {
                    ...
                    // [FIX] Old media used to be deleted unconditionally before any of the
                    // validation/copy above ran, so a too-large or failed pick permanently wiped
                    // a perfectly good existing loading screen with nothing to replace it. Only
                    // clean up the previous file(s) now that the new one has copied successfully.
                    dir.listFiles()?.forEach { if (it != destFile) it.delete() }
                    CopyOutcome.OK
                } catch (e: Exception) {
