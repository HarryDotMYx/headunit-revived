...
class MainActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // [FIX] Used to be registered as a side effect inside stopAutoConnectVideo(), which is
        // called multiple times per activity lifecycle (and not at all on some auto-connect
        // paths) — re-registering the same receiver instance on every call, with no relation to
        // that method's actual purpose. Register once here instead, same as finishReceiver above.
        ContextCompat.registerReceiver(
    ...
    fun beginAutoConnect(reason: String, mode: ConnectionUiMode, customStatusText: String? = null) {
        ...
        if (autoConnectInProgress) {
            // [FIX] Self-heal against a leaked flag: if a previous MainActivity instance
            // finished (or was killed) mid-attempt without reaching onDestroy's isFinishing
            // cleanup, autoConnectInProgress could stay stuck true forever, silently
            // disabling auto-connect for the rest of the process's life. If nothing is
            // actually connecting/connected right now and this fresh instance never started
            // its own watchdog, treat the flag as stale rather than bailing out.
            val looksStale = autoConnectWatchdog == null &&
                    !commManager.isConnected &&
                    commManager.connectionState.value is CommManager.ConnectionState.Disconnected
            if (!looksStale) return
            AppLog.w("Auto-connect: autoConnectInProgress was stuck true with no live attempt. Self-healing before starting a new one.")
        }
    ...
    private fun handleLaunchIntent(intent: Intent?) {
        ...
        if (intent.action == Intent.ACTION_VIEW) {
            if (intentData?.scheme == "headunit" && intentData.host == "connect") {
                val rawIp = intentData.getQueryParameter("ip")
                // [FIX] Reject a hostname outright instead of approving/connecting to it: DNS
                // resolves at connect time, so a hostname's target can change between when the
                // user approves "evil.attacker.com" (a far more plausible-looking prompt than a
                // raw IP) and when commManager.connect() actually resolves it — e.g. rebinding
                // to loopback or an internal host after approval. Also normalize so the same
                // address can't be stored/compared as two different-looking strings (e.g.
                // "127.1" vs "127.0.0.1").
                val ip = rawIp?.takeIf { it.isNumericIpAddress() }?.normalizedIpAddress()
                if (rawIp != null && ip == null) {
                    AppLog.w("Received connect intent with non-numeric host '$rawIp' — ignoring (hostnames are not accepted here).")
                }
                if (!ip.isNullOrEmpty()) {
                    // [FIX] Same deep-link confused-deputy issue as AutomationActivity: this is
                    // reachable by any zero-permission app/web page and used to connect to
                    // whatever IP it was given (including loopback) with no validation. Route
                    // through the same approval gate as network-discovered hosts.
                    val settings = App.provide(this).settings
                    if (settings.approvedDiscoveredHosts.contains(ip)) {
                        AppLog.i("Received connect intent for known IP: $ip")
                        ...
                        AppLog.w("Received connect intent for unapproved IP $ip — requiring approval instead of connecting directly.")
                        ContextCompat.startForegroundService(this, Intent(this, AapService::class.java).apply {
    ...
    override fun onDestroy() {
        ...
        if (isFinishing) {
            ...
            // [FIX] autoConnectInProgress/autoConnectMode/hasAdvancedToActiveState/
            // autoConnectStatusText are process-wide (@Volatile companion state) so an
            // in-flight attempt survives a config-change recreate. But if the activity is
            // actually finishing (not being recreated) while an attempt was still in
            // progress, leaving them set would wedge every future MainActivity instance in
            // this process — beginAutoConnect() bails out immediately whenever
            // autoConnectInProgress is already true. See also the self-heal check in
            // beginAutoConnect() for instances that finished without going through here.
            if (autoConnectInProgress) {
                AppLog.w("MainActivity finishing mid-auto-connect; resetting static auto-connect state.")
                autoConnectInProgress = false
