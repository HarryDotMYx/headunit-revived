...
class NetworkListFragment : Fragment(), NetworkDiscovery.Listener {
    ...
    override fun onServiceFound(ip: String, port: Int, socket: java.net.Socket?) {
        ...
        activity?.runOnUiThread {
            val ctx = context ?: run { try { socket?.close() } catch (e: Exception) {}; return@runOnUiThread }

            // [FIX] This is a second, independent implementation of the exact same
            // "connect to whatever answers on 5277" pattern already gated behind
            // approvedDiscoveredHosts in AapService.startDiscovery() — NetworkDiscovery is a
            // subnet port sweep, not an authenticated record, so any device on the LAN that
            // opens port 5277 gets auto-connected to (full AAP session: mic, GPS, screen) the
            // instant the manual "Scan" button finds it, with no confirmation. Apply the same
            // approval gate here instead of connecting/saving the address unconditionally.
            if (!App.provide(ctx).settings.approvedDiscoveredHosts.contains(ip)) {
                AppLog.w("NetworkListFragment: Found unapproved host $ip:$port. Requesting approval instead of auto-connecting.")
                try { socket?.close() } catch (e: Exception) {}
                ...
                ContextCompat.startForegroundService(ctx, Intent(ctx, AapService::class.java).apply {
                    ...
                    putExtra(AapService.EXTRA_DISCOVERED_PORT, port)
                })
    ...
    override fun onDestroyView() {
        super.onDestroyView()
        // [FIX] NetworkDiscovery.startScan() launches on its own detached
        // CoroutineScope(Dispatchers.IO) with no lifecycle tie, and scanDialog is a plain
        // AlertDialog — neither was ever torn down here, so navigating away (e.g. back press)
        // mid-scan left the subnet scan running in the background, still calling back into
        // this destroyed fragment via onServiceFound/onScanFinished, with the dialog left
        // attached to a dead view.
        networkDiscovery.stop()
        scanDialog?.dismiss()
        scanDialog = null
    }
