            ...
            if (v.id == android.R.id.button1) {
                ...
                if (App.provide(mContext).commManager.isConnected) {
                    ...
                    if (usbManager.hasPermission(device.wrappedDevice)) {
                        // [FIX] connectAndSwitch() performs blocking USB control transfers with
                        // up to 3 retries and sleeps between them (worst case several seconds,
                        // right in the "cold device" case it exists to handle) — it was being
                        // called directly from this main-thread click listener, freezing the UI
                        // and risking an ANR. Run it on IO and handle the result back on Main.
                        val usbMode = UsbAccessoryMode(usbManager)
                        scope.launch {
                            val switched = withContext(Dispatchers.IO) {
                                usbMode.connectAndSwitch(device.wrappedDevice, mSettings.useLibusb)
                            }
                            if (switched) {
