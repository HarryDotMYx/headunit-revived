...
class SettingsAdapter : ListAdapter<SettingItem, RecyclerView.ViewHolder>(SettingsDiffCallback()) { // Inherit from ListAdapter
    ...
    class SliderSettingViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(setting: SettingItem.SliderSettingEntry) {
            ...
            // [FIX] steps already guarded the degenerate valueFrom==valueTo case, but the
            // progress calculation below still divided by the raw (zero) range directly instead
            // of by this same guarded value — a 0/0 float division producing NaN/Infinity,
            // silently pinning the thumb to one end via SeekBar's internal clamp instead of
            // failing loudly. Reuse one guarded divisor for both.
            val rawRange = setting.valueTo - setting.valueFrom
            val steps = if (rawRange > 0) 100 else 1
            val range = if (rawRange > 0) rawRange else 1f

            settingSlider.max = steps
            val progress = (((setting.sliderValue - setting.valueFrom) / range) * steps).toInt().coerceIn(0, steps)
    ...
    class SegmentedButtonViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(setting: SettingItem.SegmentedButtonSettingEntry) {
            ...
            if (setting.options.size > buttons.size) {
                // [FIX] This view only has 3 button slots in its layout; anything beyond that
                // was already silently dropped (only the first 3 options ever became reachable)
                // — logging so a future caller passing more options doesn't fail invisibly.
                AppLog.w("SegmentedButtonSettingEntry '${setting.nameResId}' has ${setting.options.size} options but only ${buttons.size} button slots exist; extra options are not shown.")
            }
            ...
            visibleButtons.forEachIndexed { index, button ->
                // [FIX] `when (index) { 0 -> ...; visibleButtons.size - 1 -> ...; else -> ... }`
                // matches branches top-to-bottom, so for a single-option list (size 1,
                // "size - 1" == 0 too) index 0 always hit the *first* branch — a sole button
                // came out rounded on the left only, square on the right, instead of rounded on
                // both sides. Check the single-button case explicitly first.
                val shapeModel = when {
                    visibleButtons.size == 1 -> com.google.android.material.shape.ShapeAppearanceModel.builder()
                        .setAllCornerSizes(radius)
                        .build()
                    index == 0 -> com.google.android.material.shape.ShapeAppearanceModel.builder()
                        .setTopLeftCornerSize(radius).setBottomLeftCornerSize(radius)
                        .setTopRightCornerSize(0f).setBottomRightCornerSize(0f)
                        .build()
                    index == visibleButtons.size - 1 -> com.google.android.material.shape.ShapeAppearanceModel.builder()
