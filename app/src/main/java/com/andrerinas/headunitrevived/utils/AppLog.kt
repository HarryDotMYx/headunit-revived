...
object AppLog {
    ...
    // [FIX] init() is called from many places (App startup, AapService startup, several
    // SettingsFragment/QuickSettingsFragment toggles) with no lock. Two overlapping calls could
    // both pass the "already initialized with this source" check before either commits its
    // change, then both create a new Logger.File — the loser's FileOutputStream/BufferedWriter
    // would be silently orphaned (open, unflushed, never closed) since appLogFileLogger gets
    // overwritten before closeAppLogFileLogger() ever sees it, leaking a file descriptor for
    // the rest of the process's life. Guard the whole check-and-mutate sequence.
    fun init(settings: Settings?, context: Context? = null) = synchronized(this) {
    ...
    private fun loge(message: String, tr: Throwable?) {
        // [FIX] Log.getStackTraceString() is a pure formatting utility with no dependency on
        // which Logger backend is active — gating it on `LOGGER is Logger.Android` meant every
        // AppLog.e(msg, throwable) call silently dropped the actual stack trace whenever
        // file-based capture (Logger.File) was active, i.e. exactly when the user opted into
        // file logging for better diagnostics. Include it whenever a throwable was passed.
