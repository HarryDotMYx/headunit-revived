...
object SettingsBackupManager {
    ...
    private val backupKeys = linkedMapOf(
        ...
        // [FIX] auto-start-bt-macs doubles as the security-critical allowlist
        // NativeAaHandshakeManager checks before releasing the live WiFi Direct SSID/PSK over
        // Bluetooth (see NativeAaHandshakeManager.handleHandshake). Backing it up/restoring it
        // let a crafted settings-export file silently add an attacker's (already-paired) MAC to
        // that allowlist with no approval notification ever shown — bypassing the one-time
        // approval flow entirely. Deliberately excluded (and deliberately not added back),
        // same as the approved-discovered-hosts and approved-nearby-device-names allowlists,
        // which are also intentionally absent from this map for the same reason.
