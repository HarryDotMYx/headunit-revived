    ...
    private fun readRealMetrics(): DisplayMetrics {
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                // [FIX] View.getDisplay() returns null until the view is attached to a window —
                // always true here, since this is called from init() during inflation — so this
                // silently left m zero-initialized and onDraw() falling back to a hardcoded
                // 16:9 guess instead of the panel's real aspect ratio on every API 30+ device.
                // Context.getDisplay() doesn't require the view to be attached yet.
