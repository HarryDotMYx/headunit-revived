...
class KeyIntent(event: KeyEvent): Intent(action) {
    init {
        // [FIX] DisconnectIntent was the only one of these contract intents that scoped itself
        // with setPackage() in init — the rest relied on every call site remembering to do it
        // themselves (most did; see AutomationActivity.kt's inconsistent ACTION_FINISH_ACTIVITIES
        // sends for how easily that's missed). Set it here so the class is safe by construction
...
class LocationUpdateIntent(location: Location): Intent(action) {
    init {
        // [FIX] Carries a full precise Location; unlike the others here this one being
        // unscoped would mean a forgotten setPackage() at some future call site broadcasts the
        // driver's exact GPS position to every app on the device, not just an internal-routing
        // oversight.
        setPackage(HeadUnit.packageName)
 ...
 * but gated by the signature-level [BROADCAST_PERMISSION] (declared in AndroidManifest.xml), so
 * only components signed with the same key as this app can send or receive it.
 *
 * Other apps: must hold [BROADCAST_PERMISSION] (signature-level, so this only works for another
 * app signed with the same certificate), then
 * registerReceiver(receiver, IntentFilter(NavigationUpdateIntent.action), BROADCAST_PERMISSION, null, RECEIVER_EXPORTED).
 *
...
class NavigationUpdateIntent(
    ...
    init {
        ...
        // [FIX] This was stored via putExtra(String, Long) while every other numeric extra in
        // this class (including its own sibling EXTRA_TOTAL_DISTANCE_METERS right above) is an
        // Int — a consumer reasonably mirroring the pattern and calling getIntExtra(key, -1)
        // gets -1 forever, since Bundle doesn't coerce between numeric types on mismatched
        // getters. Store as Int like every other extra here; a trip ETA never approaches
        // Int.MAX_VALUE seconds (~68 years) so the range clamp is a formality, not a real cap.
        putExtra(EXTRA_TOTAL_TIME_SECONDS, totalTimeSeconds?.takeIf { it >= 0L }?.coerceAtMost(Int.MAX_VALUE.toLong())?.toInt() ?: -1)
